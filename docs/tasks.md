...
- [ ] [P0] `createSnippet` 関数を用意し、`title` / `body` 非空、`tags` 重複禁止、`updatedAt >= createdAt` などドメインルールをバリデーションする。
- [ ] [P0] `updateSnippet` 関数を実装し、partial 更新値を受けて差分マージと再バリデーションを行う。
...
- [ ] [P0] `CreateSnippetUseCase` を実装し、入力 DTO→`createSnippet`→保存→結果返却のフローを整備する。
- [ ] [P0] `UpdateSnippetUseCase` で差分マージと `updatedAt` 更新、ReadOnly ライブラリチェックを行う。
